...
function sanitizeStateForRead(state, actor) {
  ...
  const staffUserIds = new Set(
    (state.users || [])
      .filter((user) => (user.roles || []).some((role) => ["director", "coach", "delegate"].includes(role)))
      .map((user) => user.id)
  );
  ...
  copy.events = (copy.events || []).filter((event) => visibleTeams.has(event.teamId) || (event.playerIds || []).some((id) => visiblePlayers.has(id)));
  copy.trainings = (copy.trainings || []).filter((training) => visibleTeams.has(training.teamId) || (training.playerIds || []).some((id) => visiblePlayers.has(id)));
  copy.callups = (copy.callups || []).filter((callup) => visibleTeams.has(callup.teamId) || (callup.playerIds || []).some((id) => visiblePlayers.has(id)));
